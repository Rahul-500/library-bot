const { beginTransaction, commitTransaction, rollbackTransaction } = require('../../src/service/transactions')
...
describe('Transaction Functions', () => {
    ...
    test('beginTransaction rejects on error', async () => {
        ...
        await expect(beginTransaction(mockConnection)).rejects.toThrow('Failed to begin transaction');
        expect(mockConnection.beginTransaction).toHaveBeenCalled();
    });
    ...
    test('commitTransaction rejects on error', async () => {
        ...
        await expect(commitTransaction(mockConnection)).rejects.toThrow('Failed to commit transaction');
        expect(mockConnection.commit).toHaveBeenCalled();
    });
    ...
    test('rollbackTransaction rejects on error', async () => {
        ...
        await expect(rollbackTransaction(mockConnection)).rejects.toThrow('Failed to rollback transaction');
        expect(mockConnection.rollback).toHaveBeenCalled();
    });
});
